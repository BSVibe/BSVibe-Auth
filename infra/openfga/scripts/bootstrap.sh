...
#   2. Compute SHA-256 of infra/openfga/bsvibe.fga.
#   3. Compare against the latest applied schema sha (cached in .bootstrap.json).
#   4. If different (or no model yet) → write new model → record AUTH_MODEL_ID.
#   5. Persist STORE_ID + AUTH_MODEL_ID + schema_sha into
#      infra/openfga/.bootstrap.json (gitignored, mode 0600).
#
...
#
# Usage:
#   ./infra/openfga/scripts/bootstrap.sh                # uses infra/openfga/.env
#   ./infra/openfga/scripts/bootstrap.sh --force        # rewrite model even if hash matches
#   OPENFGA_API_URL=http://... ./infra/openfga/scripts/bootstrap.sh  # remote target

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
OPENFGA_DIR="${OPENFGA_DIR:-$(cd "$SCRIPT_DIR/.." && pwd)}"
SCHEMA_FILE="$OPENFGA_DIR/bsvibe.fga"
...
if ! curl -sf "$API/healthz" >/dev/null; then
  err "OpenFGA $API not reachable — start docker-compose first (./infra/openfga/scripts/up.sh)"
fi
