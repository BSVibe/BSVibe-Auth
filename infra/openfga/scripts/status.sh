...
#   2 = bootstrap state missing or stale (run ./infra/openfga/scripts/bootstrap.sh)
...
if [ ! -f "$STATE_FILE" ]; then
  printf '%-12s %s\n' "state" "missing — run ./infra/openfga/scripts/bootstrap.sh"
  exit 2
fi
...
if echo "$RESP" | jq -e '.authorization_model.id' >/dev/null 2>&1; then
  ...
  printf '%-12s %s\n' "remote" "MISSING — re-run ./infra/openfga/scripts/bootstrap.sh"
  exit 2
fi
